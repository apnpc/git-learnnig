---
title: "多存储库"
description: "克隆本地仓库并理解 origin 与远程跟踪引用。"
type: concept
group: "基础篇"
sidebar:
  order: 27
...
本节创建一个名为 `cloned_git_learning` 的新存储库，展示如何在两个仓库之间同步更改。
...
# 在仓库目录的上级目录下输入
$ git clone git_learning cloned_git_learning
Cloning into 'cloned_git_learning'...
done.
```

## 进入 `cloned_git_learning` 查看提交日志

```powershell
# 进入文件夹
$ cd .\cloned_git_learning\

# 查看日志
git hist
* 2456c71 2023-05-05 | Added test2.txt (HEAD -> master, origin/master, origin/lgnew, origin/HEAD) [aku]
* 1c78eab 2023-05-05 | Added test3.txt [aku]
* 31e5621 2023-05-05 | Added test2.txt [aku]
...
`origin` 是 `git clone` 为源仓库创建的默认远程名称。单独执行 `git init` 不会自动创建远程仓库；可以使用 `git remote add <name> <url>` 自行添加。
