...
在 Git 中，后缀 `^` 表示提交的第一个父提交，`^2` 表示合并提交的第二个父提交。它可以与分支名、标签名或提交 ID 一起使用。
...
- `abc123^`：引用对象 ID 缩写为 abc123 的提交的父提交。

除了单个 `^` 符号外，还可以使用多个 `^` 符号来引用更早的提交。例如，`HEAD^^` 表示当前提交的父提交的父提交，即当前提交之前的第二个提交。

`~n` 表示连续沿第一个父提交向前移动 n 次。例如，`HEAD~3` 是沿第一父链向前三步；`HEAD~2^2` 是先沿第一父链向前两步，再选择该提交的第二个父提交。
`^` 可以与分支名、标签名或提交 ID 一起使用，以选择指定父提交。
