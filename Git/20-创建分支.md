---
title: "创建分支"
description: "创建并切换分支，在独立分支上提交更改。"
type: tutorial
group: "基础篇"
sidebar:
  order: 20
...
在 Git 中，分支是指向提交的可移动引用。初始分支可能叫 `main`、`master` 或团队指定的其他名称；本实验沿用 `master`。
...
> 旧项目中也常见 `git checkout -b lgnew`，效果等同于创建并切换分支。
...
# 提交
git commit -m "Added test3.txt"
[master 1c78eab] Added test3.txt
1 file changed, 0 insertions(+), 0 deletions(-)
...
```powershell
git hist --all
* 1c78eab 2023-05-05 | Added test3.txt (HEAD -> master) [aku]
| * 3d99c8f 2023-05-05 | Added test2.txt (lgnew) [aku]
|/
