---
title: "安装 Git"
description: "说明在 Windows 上安装 Git 时的主要选项和验证方法。"
type: tutorial
group: "基础篇"
sidebar:
  order: 4
...
![4](images/04-install-04.png)

## 选择 Git 使用的默认编辑器

![5](images/04-install-05.png)

当 Git 需要使用编辑器时会调用这个选项，例如填写提交信息时。

## 主分支名称选择

![6](images/04-install-06.png)

- 第一项，让 Git 自行设置，默认名称 master。
- 第二项是自定义初始分支名称。Git 2.28 增加了 `init.defaultBranch` 配置，但 Git 本身的默认名称仍可能是 `master`；GitHub、安装器或团队配置也可能选择 `main`。本教程示例沿用历史实验中的 `master`，新项目应遵循团队约定。

## 添加环境变量

![7](images/04-install-07.png)
...
当使用 HTTPS 访问远程仓库时，通常需要 HTTPS URL，并通过凭据管理器、浏览器登录或访问令牌认证。GitHub 等平台不再接受账户密码进行 Git 操作。不要把令牌直接写进 URL 或提交到仓库。
...
![12](images/04-install-12.png)

`git pull` 先获取远程更新，再根据参数和配置执行快进、合并或变基。团队应明确选择策略；不确定时先使用 `git fetch` 检查差异。
...
选择 "only ever fast-forward" 后，只要本地和上游历史已经分叉，`git pull` 就会停止；即使文件内容不存在冲突也一样。此时应检查提交图，再明确选择 merge 或 rebase。

## 选择个人凭据认证辅助工具

![13](images/04-install-13.png)
