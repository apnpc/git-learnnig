---
title: "忽略文件"
description: "编写 .gitignore 规则并排查文件未被跟踪的原因。"
type: reference
group: "基础篇"
sidebar:
  order: 18
...
[master 5dc8b1e] Added .gitignore
...
`.gitignore` 只影响尚未跟踪的文件。文件已经提交时，需要先从索引移除，再提交这次变更：

```powershell
git rm --cached <file>
```

使用 `git check-ignore -v <file>` 可以查看是哪条规则忽略了文件。
